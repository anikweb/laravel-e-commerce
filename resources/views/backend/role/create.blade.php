                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            @if (session('permissionErr'))
                                                <div class="text-danger text-bold"><i class="fa fa-exclamation-circle"></i> {{ session('permissionErr') }}</div>
                                            @endif
                                            @foreach ($permissions as $permission)
                                                <div class="custom-control custom-checkbox">
                                                    <input {{ old('permission[]') }} class="custom-control-input" type="checkbox" id="customCheckbox{{ $permission->id }}" value="{{ $permission->name }}" name="permission[]" ><label for="customCheckbox{{ $permission->id }}" class="custom-control-label">{{ $permission->name }}</label>
                                                </div>
                                            @endforeach
                                        </div>
                                    </div>
