...
class CartControllers extends Controller
{
    ...
    function cartPost(Request $request){
        ...
        if(Cart::where('cookie_id',$request->cookie('cookie_id'))->where('product_id',$request->product_id)->where('color_id',$request->color_id)->where('size_id',$request->size_id)->exists()){
            Cart::where('cookie_id',$request->cookie('cookie_id'))->where('product_id',$request->product_id)->where('color_id',$request->color_id)->where('size_id',$request->size_id)->increment('quantity',$request->quantity);
        }else{
            $cart = new Cart;
